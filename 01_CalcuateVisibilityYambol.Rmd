...
# Print figure
tiff("figures/03.tiff", width = 9, height = 6, units = "in", res = 300)
profile(9412, 8242)
dev.off()
```
...
## Chronology
Downloading the mound and burial data for each of the two litarute corpus datasets, Izvestia and AOR. 
```{r mnds-google}
...
all %>% 
  filter(!is.na(StartDate)) %>% 
  pull(StartDate) %>% table() %>% plot(ylab = "Grave count", main = "Temporal span of 295 burial assemblages from 41 mounds in Yambol")

```
### printing
```{r eval = FALSE}
tiff( "figures/02.tiff", width = 8, height = 5, units = "in", res = 300)
all %>% 
  filter(!is.na(StartDate)) %>% 
  pull(StartDate) %>% table() %>% plot(ylab = "Grave count", main = "Temporal span of 295 burial assemblages from 41 mounds in Yambol")
dev.off()
